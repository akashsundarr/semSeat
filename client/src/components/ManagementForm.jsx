...
// A reusable notification component (no changes here)
...
function ManagementForm({ title, fields, onSubmit }) {
  const initialFormState = fields.reduce((acc, field) => {
    // For multi-select, initialize with an empty array
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md border w-full max-w-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        {fields.map((field) => (
          <div key={field.name}>
            ...
            {field.type === 'multi-select' ? (
              ...
              >
                <option value="" disabled>Select a {field.label}</option>
                {field.options.map(option => (
                  <option key={option.value} value={option.value}>{option.label}</option>
                ))}
              </select>
        ...
        <button type="submit" disabled={isLoading} className="w-full px-4 py-2 bg-blue-600 text-white font-semibold rounded-md hover:bg-blue-700 disabled:bg-gray-400">
