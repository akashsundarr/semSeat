...
// The main allocation function - REDESIGNED LOGIC
router.post("/run-allocation", async (req, res) => {
    const { series_id, allocation_date, start_time } = req.body;
    if (!series_id || !allocation_date || !start_time) {
        return res.status(400).json({ error: "series_id, allocation_date, and start_time are required." });
    ...
    try {
        ...
        // 1. Get all exams and their subject IDs for the specific time slot
        const [scheduledExams] = await conn.query(
            `SELECT exam_id, subject_id FROM ScheduledExams WHERE series_id = ? AND exam_date = ? AND start_time = ?`,
            [series_id, allocation_date, start_time]
        );
        ...
        // 2. Get all eligible students and GROUP THEM BY SUBJECT
        const [students] = await conn.query(
            `SELECT stu.student_id, stu.batch, stu.semester, d.dept_code, s.subject_id
             FROM Students stu
             JOIN Subjects s ON stu.semester = s.semester AND stu.dept_id = s.dept_id
             JOIN Departments d ON stu.dept_id = d.dept_id
             WHERE s.subject_id IN (?) AND stu.status IN ('Active', 'Unassigned')
             ORDER BY stu.semester, d.dept_code, stu.batch, stu.student_id`, // Predictable order
            [subjectIdsForSlot]
        );
        
        const studentsBySubject = new Map();
        for (const student of students) {
            if (!studentsBySubject.has(student.subject_id)) {
                studentsBySubject.set(student.subject_id, []);
            }
            studentsBySubject.get(student.subject_id).push(student);
        }

        // 3. Get all classrooms and create a virtual layout
        const [rooms] = await conn.query("SELECT room_id, capacity FROM Classroom ORDER BY room_id");
        ...
        for (const room of rooms) {
            ...
            for (let i = 0; i < benches; i++) {
                benchLayout.push({
                    row_number, bench_number,
                    left: null, middle: null, right: null,
                });
                bench_number++;
                if (bench_number > 7) { // Assuming 7 benches per row
                    bench_number = 1;
                    row_number++;
                }
            }
            roomLayout.set(room.room_id, { benches: benchLayout });
        }

        // 4. Clear any previous allocations for this specific exam session
        await conn.query(`DELETE FROM Allocations WHERE exam_id IN (?)`, [examIdsForSlot]);

        // 5. Run the new seating algorithm
        const studentSeatMap = new Map(); // Final map: { student_id -> { room_id, ... } }
        const seatPositions = ["left", "middle", "right"];

        // Iterate through each subject's student pool
        for (const [subjectId, studentPool] of studentsBySubject.entries()) {
            // Iterate through each seat position pass (Pass 1: left, Pass 2: middle, etc.)
            for (const position of seatPositions) {
                if (studentPool.length === 0) break; // No more students for this subject

                // Iterate through every bench in every room
                for (const [roomId, room] of roomLayout.entries()) {
                    if (studentPool.length === 0) break;

                    for (const bench of room.benches) {
                        if (studentPool.length === 0) break;

                        // Check if the current seat position is available
                        if (bench[position] === null) {
                            // VALIDATION: Prevent placing same subject students next to each other
                            const isLeftOccupiedBySame = bench.left && bench.left.subject_id === subjectId;
                            const isMiddleOccupiedBySame = bench.middle && bench.middle.subject_id === subjectId;

                            if (position === "middle" && isLeftOccupiedBySame) continue;
                            if (position === "right" && (isLeftOccupiedBySame || isMiddleOccupiedBySame)) continue;

                            // Allocate the student
                            const studentToAllocate = studentPool.shift();
                            bench[position] = { student_id: studentToAllocate.student_id, subject_id: subjectId };

                            // Record the final seat assignment
                            studentSeatMap.set(studentToAllocate.student_id, {
                                room_id: roomId,
                                row_number: bench.row_number,
        ...
        // 6. --- BULK INSERT (Still Optimized) ---
        const assignedStudentIds = Array.from(studentSeatMap.keys());
        const allocationInserts = [];
        for (const student_id of assignedStudentIds) {
            const seat = studentSeatMap.get(student_id);
            const exam_id = subjectToExamMap.get(students.find(s => s.student_id === student_id).subject_id);
            if (exam_id) {
                allocationInserts.push([
                    student_id, exam_id, seat.room_id, seat.row_number,
                    seat.bench_number, seat.seat_position
        ...
        const unassignedStudents = [].concat(...Array.from(studentsBySubject.values()));
        if (unassignedStudents.length > 0) {
            const unassignedStudentIds = unassignedStudents.map(s => s.student_id);
            await conn.query("UPDATE Students SET status = 'Unassigned' WHERE student_id IN (?)", [unassignedStudentIds]);
        }
        ...
        res.json({
            ...
            message: `Allocation completed for ${allocation_date} at ${start_time}.`,
            students_assigned: assignedStudentIds.length,
            unassigned_count: unassignedStudents.length,
        });
